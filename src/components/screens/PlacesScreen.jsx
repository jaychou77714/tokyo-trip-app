    ...
    <div className="paper-bg min-h-screen pb-24">
      <div className="px-5 pt-12 pb-6 max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-2.5 mb-5">
          <CategoryCard active={activeCategory === 0} onClick={() => setActiveCategory(0)} emoji="✦" name="全部" count={PLACES.length} color="#3D2817" />
          {CATEGORIES.map(cat => (
            <CategoryCard key={cat.id} active={activeCategory === cat.id} onClick={() => setActiveCategory(cat.id)}
              emoji={cat.emoji} name={cat.name} count={getPlacesByCategory(cat.id).length} color={cat.color} />
          ))}
        </div>
          ...
          <div className="mb-5" style={{ border: '1.5px solid #3D2817', boxShadow: '3px 3px 0 #3D2817' }}>
            <MapView
              ...
              markers={filtered.map(p => ({
                ...
                popup: { title: p.name_zh, subtitle: p.type, action: { label: '查看詳情', onClick: () => setSelectedPlace(p) } }
              }))}
            />
          </div>
        )}
