...
import { ArrowLeft, Plus, MapPin, Trash2, Clock, Share2, UserPlus, Copy, Check } from 'lucide-react'
import { Button, Modal, Input, Textarea, Select, EmptyState } from '../Common'
import MapView from '../MapView'
import { listItinerary, saveItineraryItem, deleteItineraryItem, listTripMembers } from '../../lib/storage'
        ...
        {dayItems.length === 0 ? (
          ...
          <div className="space-y-2">
            {dayItems.map((item, idx) => (
              <ItineraryCard
                key={item.id} item={item} num={idx + 1}
                memberMap={memberMap}
...
function ItineraryCard({ item, num, memberMap, onEdit, onDelete, onMoveDay, totalDays }) {
  const place = item.place_id ? getPlaceById(item.place_id) : null
  ...
  return (
    <Modal open={open} onClose={onClose} title="✿ 邀請成員加入" maxWidth="max-w-md">
      <div className="px-5 py-4">
          ...
          <div className="space-y-1.5">
            {members.map(m => (
              <div key={m.id} className="flex items-center gap-2 paper-plain p-2"
                ...
                {m.role === 'owner' && (
                  <span className="text-[9px] text-shu font-mono">建立者</span>
                )}
              </div>
            ))}
          </div>
        ...
        <p className="text-[11px] text-usuzumi mt-4 leading-relaxed font-display italic">
          ※ 連結傳給朋友，他們點開輸入暱稱即可加入。<br />
          ※ 加入後所有人都能編輯行程內容。
        </p>
      </div>
    </Modal>
    ...
    <Modal open={open} onClose={onClose} title={item?.id ? '編輯行程項目' : '加入行程'}>
      <div className="px-5 py-4 space-y-3">
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          />
          <Input
            label="到達時間" type="time"
            value={form.start_time || ''}
            onChange={(e) => setForm({ ...form, start_time: e.target.value })}
          />
          <Input
            label="停留分鐘" type="number" min="0" step="15"
