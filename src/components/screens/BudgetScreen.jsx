  ...
  return (
    <div className="paper-bg min-h-screen pb-24">
      <div className="px-5 pt-12 pb-4 max-w-4xl mx-auto">
        ...
        {tab === 'expenses' && (
          <ExpensesTab
            expenses={expenses} byCategory={byCategory} rate={rate} memberMap={memberMap}
            onAdd={() => setEditingExp({})}
      ...
      {expenses.length === 0 ? (
        ...
        <div className="space-y-1.5">
          {expenses.map(exp => {
            ...
            return (
              <div key={exp.id}
                className="group flex items-start gap-3 paper-plain p-3 transition-all"
  ...
  return (
    <div>
      ...
      {items.length === 0 ? (
        ...
        <div className="space-y-1.5">
          {items.map(item => (
            <div key={item.id} className="group flex items-start gap-3 paper-plain p-3"
            ...
            <div className="paper-plain p-2 mb-2 text-[11px] font-display" style={{ border: '1px dashed #D4B896' }}>
              ...
              <div className="flex flex-wrap gap-1">
                ...
                <button type="button" onClick={() => setSplitMembersStr(memberNames)}
                  className="text-[10px] px-2 py-0.5 text-usuzumi border border-dashed border-usuzumi">
                  全部加入分帳
                </button>
              </div>
            </div>
