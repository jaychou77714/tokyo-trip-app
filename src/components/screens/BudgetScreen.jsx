...
export default function BudgetScreen({ trips, selectedTripId, onSelectTrip, showToast }) {
...
function BudgetForTrip({ trip, trips, onSelectTrip, showToast }) {
  ...
  async function loadAll() {
    ...
    setExpenses(exp); setTaxFree(tax)
  }
  ...
  return (
    <div className="paper-bg min-h-screen pb-24">
      <div className="px-5 pt-12 pb-4 max-w-4xl mx-auto">
        ...
        {tab === 'expenses' && (
          <ExpensesTab
            expenses={expenses} byCategory={byCategory} rate={rate}
            onAdd={() => setEditingExp({})}
      ...
      <ExpenseEditModal
        open={editingExp !== null} item={editingExp}
        onClose={() => setEditingExp(null)}
        onSave={async (form) => { await saveExpense({ ...editingExp, ...form }, trip.id); setEditingExp(null); showToast('✓ 已儲存', 'success'); loadAll() }}
      />

      <TaxEditModal
        open={editingTax !== null} item={editingTax}
        onClose={() => setEditingTax(null)}
        onSave={async (form) => { await saveTaxFree({ ...editingTax, ...form }, trip.id); setEditingTax(null); showToast('✓ 已儲存', 'success'); loadAll() }}
      />
    </div>
  )
}
...
function ExpensesTab({ expenses, byCategory, rate, onAdd, onEdit, onDelete }) {
  return (
    <div>
      {byCategory.length > 0 && (
      ...
      {expenses.length === 0 ? (
        ...
        <div className="space-y-1.5">
          {expenses.map(exp => {
            ...
            return (
              <div key={exp.id}
                className="group flex items-center gap-3 paper-plain p-3 transition-all"
...
function TaxTab({ items, onAdd, onEdit, onDelete }) {
  ...
  return (
    <div>
      ...
      {items.length === 0 ? (
        ...
        <div className="space-y-1.5">
          {items.map(item => (
            <div key={item.id} className="group flex items-center gap-3 paper-plain p-3"
              style={{ border: '1.5px solid #3D2817' }}>
              ...
              <button onClick={() => onDelete(item)} className="opacity-0 group-hover:opacity-100 text-stamp p-1">
                <Trash2 size={12} />
              </button>
            </div>
          ))}
...
function SettleTab({ settlement, rate }) {
  ...
  return (
    <div>
      ...
      <div className="text-[11px] text-usuzumi mt-4 leading-relaxed font-display italic">
        ※ 演算法以「最少轉帳次數」原則計算。<br />
