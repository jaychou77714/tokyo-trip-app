...
import { ArrowLeft, Plus, Trash2, RotateCcw, Sparkles } from 'lucide-react'
...
export default function ChecklistScreen({ trips, onBack, showToast }) {
  ...
  return <ChecklistForTrip trip={trip} trips={trips} onSelectTrip={setSelectedTripId} onBack={onBack} showToast={showToast} />
}

function ChecklistForTrip({ trip, trips, onSelectTrip, onBack, showToast }) {
  ...
  async function load() {
    setLoading(true)
    const data = await listChecklist(trip.id)
    setItems(data)
    setLoading(false)
  }

  async function handleToggle(item) {
    await saveChecklistItem({ ...item, is_done: !item.is_done }, trip.id)
    load()
  }
  ...
  async function handleLoadTemplate() {
    ...
    await bulkInsertChecklist(initial, trip.id)
    setConfirmReset(false)
    showToast(`✿ 已載入 ${CHECKLIST_TEMPLATE.length} 項預設清單`, 'success')
    load()
  }
  ...
  return (
    <div className="paper-bg min-h-screen pb-24">
      <div className="px-5 pt-12 pb-6 max-w-3xl mx-auto">
        ...
        {/* 倒數卡片 */}
        <div
          className="paper-plain p-5 mb-5 relative"
          style={{ border: '2px solid #3D2817', boxShadow: '4px 4px 0 #FF8B5A' }}
        >
          <div
            className="absolute -top-2 left-6 w-20 h-4"
            style={{
              background: '#FF8B5A',
      ...
      <ConfirmDialog
        open={confirmReset}
        title="載入預設範本"
        message={`會新增 ${CHECKLIST_TEMPLATE.length} 項預設清單到這個行程，現有項目會保留。`}
        confirmText="載入"
...
function ChecklistItemRow({ item, onToggle, onDelete }) {
  return (
    <div
      className="group flex items-start gap-3 paper-plain p-2.5 transition-all"
      style={{
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className={`text-sm font-display ${item.is_done ? 'line-through text-usuzumi' : ''}`}>
          {item.item_name}
          {item.is_custom && (
            <span className="ml-2 text-[9px] text-shu font-mono">+自訂</span>
          )}
        </div>
        {item.item_desc && (
          <div className="text-[11px] text-usuzumi mt-0.5">{item.item_desc}</div>
        )}
      </div>

      <button
        onClick={onDelete}
        className="opacity-0 group-hover:opacity-100 text-stamp p-1 transition-opacity flex-shrink-0"
      >
        <Trash2 size={12} />
      </button>
    </div>
  ...
  return (
    <Modal open={open} onClose={onClose} title="新增 Checklist 項目">
      <div className="px-5 py-4 space-y-3">
        ...
        <Input
          label="補充說明（可選）"
          placeholder="例：飯店宵夜備用"
          value={form.item_desc || ''}
          onChange={(e) => setForm({ ...form, item_desc: e.target.value })}
        />
