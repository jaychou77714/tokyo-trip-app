...
import {
  ...
  saveItineraryItem, findTripByShareCode, joinTrip
} from './lib/storage'
...
export default function App() {
  ...
  // 共編相關
  const [joinPending, setJoinPending] = useState(null) // 解析到的待加入行程
  ...
  async function handleLogin(u) {
    ...
    // 如果 URL 有 ?join=，登入後直接進入確認流程
    if (joinPending) {
      // joinPending 已存在，等用戶按確認
    }
  }
  ...
  return (
    <div className="min-h-screen">
      ...
      {view === 'home' && <HomeScreen user={user} trips={trips} onSelectTrip={handleSelectTrip} showToast={showToast} onRefresh={refreshTrips} />}
      ...
      <div className="max-w-3xl mx-auto flex relative">
        {tabs.map((t) => {
          ...
          return (
            ...
            >
              {active && (
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-8 h-1.5" style={{ background: '#FF8B5A' }} />
              )}
